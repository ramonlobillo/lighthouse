...
class kpi:
    ...
    def __init__(self,kpis_config_filepath,period,logfilepath):
        ...
        self.now=datetime.datetime.strptime("2019-10-14 23:59:59,999","%Y-%m-%d %H:%M:%S,%f")
        self.limit=now - datetime.timedelta(seconds=int(period))

        self.considered_log=self.getLogs(self.logfilepath, self.limit)
    # END constructor
        ...
        with open(self.logfilepath, 'rt') as log:
            for line in reversed(log.readlines()):
                if timestamp_format.search(" ".join(line.split()[0:2])):
                    timestamp=datetime.datetime.strptime((" ".join(line.split()[0:2])), "%Y-%m-%d %H:%M:%S,%f")
                    if timestamp > self.limit:
                        considered_log.append(line)
                    else:
                        break
        return loglines
    # END getlogs
